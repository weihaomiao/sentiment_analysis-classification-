...
def get_order(n_samples):
    ...
    file_path = os.path.join(data_dir, str(n_samples) + '.txt')
    try:
...
def pegasos_single_step_update(
    ...
    if margin <= 1:  # only update if hinge loss > 0
        theta = theta + eta * (label * feature_vector - L * theta)
        theta_0 = theta_0 + eta * label
    else:  # no loss, only regularization
...
def classifier_accuracy(
    ...
    """
    # Your code here
    thetas, theta_0 = classifier(train_feature_matrix, train_labels, **kwargs)
    train_preds = classify(train_feature_matrix, thetas, theta_0)
    train_accuracy = accuracy(train_preds, train_labels)
    val_preds = classify(val_feature_matrix, thetas, theta_0)
